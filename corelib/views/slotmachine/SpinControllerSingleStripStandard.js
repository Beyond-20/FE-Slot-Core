...
class SpinControllerSingleStripStandard {
    constructor() {
        this.symbolsArray = [];
        this.stripsArray = [];
    ...
    startAllSpin (sticky) {
        ...
        this.newtimeoutIds = [];
        this.spinningReels = [];
        this.isReelStripSpinning = false;
    ...
    startReelStripSpin (index) {
        if (CoreLib.Model.GameConfig.isTurboOn) {
            this.newtimeoutIds[index] = setTimeout(this.notifyStart.bind(this, index), index * 0);
        } else {
            this.newtimeoutIds[index] = setTimeout(this.notifyStart.bind(this, index), index * CoreLib.Util.getDefaultValue(this.slotMachine.configData.data.reelStartGap, 100));
        }

    }
    clearNewTimeoutIds() {
        let len = this.newtimeoutIds.length;
        for (let k = 0; k < len; k++) {
            clearTimeout(this.newtimeoutIds[k]);
        }
    }
    notifyStripStart(index) {
        this.stopStarted = [];
        let symbslen = this.symbolsArray[0].length - 1;
        this.isReelStripSpinning = true;
        for (let k = 0; k < this.slotMachine.configData.data.noOfReels; k++) {
            this.stopIndexes.push(symbslen);
            this.reelsStopped.push(0);
            this.speeds.push(0);
        ...
        CoreLib.AnimationManager.animateTween(this.stripsArray[index], 1.8, { y : 0, repeat : -1, ease : Linear.easeNone});
    }
    notifyStart (index) {
    ...
    doFinalStop (reelno) {
        ...
        if (this.spinStopIndex >= this.slotMachine.configData.data.noOfReels) {
            this.ticker.stop();
            if (this.isReelStripSpinning) {
                CoreLib.EventHandler.dispatchEvent(CoreLib.SlotEvents.REELSTRIP_SINGLE_SPIN_COMPLETED, reelno);
            } else {
                CoreLib.EventHandler.dispatchEvent(CoreLib.SlotEvents.REELSTRIP_SPIN_COMPLETED, reelno);
            }
...
export const spinControllerStrip = new SpinControllerSingleStripStandard();

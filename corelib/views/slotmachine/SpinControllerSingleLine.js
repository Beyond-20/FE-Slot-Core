...
class SpinControllerSingleLine {
    constructor() {
        this.symbolsArray = [];
        this.stripsArray = [];
    ...
    setSlotInstance (instance) {
        ...
        this.stopSpinInterval = [];
    }

    startAllSpin (sticky) {
        clearTimeout(this.notifystartId);
        if (this.stopSpinInterval.length > 0) {
            for (let k = 0; k < this.stopSpinInterval.length; k ++) {
                clearInterval(this.stopSpinInterval[k]);
            }
        }
        this.stopSpinInterval = [];
        ...
        CoreLib.EventHandler.dispatchEvent("SPIN_STARTED");
        //this.ticker.start();
    ...
    showSymbols (index) {
        let len = this.symbolsArray[index].length;
        for (let k = 0; k < len; k++) {
            this.symbolsArray[index][k].showSymbol();
            this.symbolsArray[index][k].visible = true;
        }
    }
    startReelStripSpin (index) {
        if (CoreLib.Model.GameConfig.isTurboOn) {
            this.notifystartId = setTimeout(this.notifyStart.bind(this, index), index * 0);
        } else {
            this.notifystartId = setTimeout(this.notifyStart.bind(this, index), index * CoreLib.Util.getDefaultValue(this.slotMachine.configData.data.reelStartGap, 100));
        }
    ...
    stopReelStripQuickForPress(reelno, reelview, isTurbo = false) {
        ...
        for (let k = reellen - 1; k >= 0; k--) {
            ...
            this.slotMachine.symbolsContainer.addChild(symb);
            CoreLib.AnimationManager.killTweensOf(symb);
            let cursymb = this.getResultSymbolNumber(reelno);
            if (cursymb == "BL") {
                cursymb = this.slotMachine.getNewRandomSymbolNumber();
                this.blankSymbolReel[reelno] = true;
            }
    ...
    stopReelStrip (reelno, reelview, isTurbo = false) {
        ...
        this.resultSymbols[reelno].unshift(this.slotMachine.getNewRandomSymbolNumber());
        this.resultSymbols[reelno].push(this.slotMachine.getNewRandomSymbolNumber());
        ...
        if (reelno == this.slotMachine.configData.data.noOfReels - 1) {
            ...
            for (let k = 0; k < len; k++) {
                if (this.stickyReels && this.stickyReels.indexOf(k) > -1) {
                    ...
                    if (isTurbo) {
                        this.stopSpinInterval[k] = setTimeout(this.callToStopStrip.bind(this, k), k * 5);
                    } else {
                        ...
                        if (this.slotMachine.checkAnticipation(k) > 1) {
                            ...
                            this.stopSpinInterval[k] = setTimeout(this.callToStopStrip.bind(this, k), k * CoreLib.Util.getDefaultValue(this.slotMachine.configData.data.reelStopGap, 200));
                        }
                    }
                }
            }
    ...
    startAnticipation (reelno) {
        ...
        this.stopAntiTime = setTimeout(this.handleAnticiptionStop.bind(this, reelno), CoreLib.Util.getDefaultValue(this.slotMachine.configData.data.anticipationDelay, 3000));
    ...
    handleAnticiptionStop (reelno) {
        ...
        if (reelno < this.slotMachine.configData.data.noOfReels - 1) {
            this.handleAntiTime = setTimeout(this.handleAnticiptionStop.bind(this, reelno + 1), CoreLib.Util.getDefaultValue(this.slotMachine.configData.data.anticipationDelay));
        }
    }
    ...
    forceStopReel (reelno) {
        ...
        let len = this.symbolsArray[reelno].length;
        for (var k = 0; k < len; k++) {
            this.slotMachine.symbolsContainer.addChild(this.symbolsArray[reelno][k]);
            if (k === 0) {
    ...
    showEndJerk (reelno, isTurbo) {
        ...
        if (isTurbo) {
            ...
            duration = 0.25;
            for (var k = 0; k < len; k++) {
                if (slotModel.getReelsView()[reelno][0] == "BL") {
                    finaly = (k - 1) * (this.slotMachine.configData.data.symbolHeight) + this.slotMachine.configData.data.symbolHeight / 2;
                } else {
                    finaly = (k - 1) * (this.slotMachine.configData.data.symbolHeight)
                }
                this.slotMachine.symbolsContainer.addChild(this.symbolsArray[reelno][k]);
                if (k === 0) {
                    CoreLib.AnimationManager.animateTween(this.symbolsArray[reelno][k],duration, {y : finaly, ease : customEase, onComplete : this.doFinalStop.bind(this, reelno)});
                } else {
                    CoreLib.AnimationManager.animateTween(this.symbolsArray[reelno][k], duration, {y: finaly, ease: customEase});
                }
            }
        } else {
            duration = 1;
        ...
        if (CoreLib.Model.GameConfig.reelStopDelay != undefined) {
            this.stopEndJerkTime = setTimeout(this.playReelStopSound.bind(this), CoreLib.Model.GameConfig.reelStopDelay);
        } else {
            this.stopEndJerkTime = setTimeout(this.playReelStopSound.bind(this), 200);
        }
    ...
    doFinalStop (reelno) {
        ...
        if (this.spinStopIndex >= this.slotMachine.configData.data.noOfReels) {
            this.ticker.stop();
            this.adjustSymbols();
            this.stopFinalTime = setTimeout(this.notifyEnd.bind(this, reelno), 0);
        }
        this.stripsArray[reelno].visible = false;
        this.showSymbols(reelno);
    ...
    adjustSymbols() {
        let finaly;
        for (let reelno = 0; reelno < this.slotMachine.configData.data.noOfReels; reelno++) {
            let len = this.symbolsArray[reelno].length;
            for (var k = 0; k < len; k++) {
                this.slotMachine.symbolsContainer.addChild(this.symbolsArray[reelno][k]);
                if (slotModel.getReelsView()[reelno][0] === "BL") {
                ...
                this.symbolsArray[reelno][k].y = finaly;
            }
        }
    }
    notifyEnd (reelno) {
        clearTimeout(this.notifystartId, this.stopAntiTime, this.handleAntiTime, this.stopEndJerkTime, this.stopFinalTime);
        for (let k = 0; k < this.configData.data.noOfRows; k++) {
            CoreLib.AnimationManager.killTweensOf(this.symbolsArray[reelno][k]);
            CoreLib.AnimationManager.killTweensOf(this.stripsArray[k]);
        }
        if (this.stopSpinInterval.length > 0) {
            for (let k = 0; k < this.stopSpinInterval.length; k ++) {
                clearInterval(this.stopSpinInterval[k]);
    ...
    showLandingAnimation (symb, reelno) {
        ...
        // adding this parameter which will not cause any regression to older games as it is not handled in the core class
        // this parameter will be handled in overridden game level reelslotsymbol class to avoid any regression
        symb.showLandingAnimation(reelno);
    }
...
export const spinControllerSL = new SpinControllerSingleLine();
